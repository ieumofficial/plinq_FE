...
type Props = {
  ...
  current: ProjectRoleDb
  onClose: () => void
  onSave: (next: ProjectRoleDb) => void
}

const OPTIONS: { key: ProjectRoleDb; label: string; bg: string; desc: string }[] = [
  {
    key: 'admin',
    label: 'Admin',
    bg: 'bg-gray-main',
    desc: 'Full access — manage members, settings, and tasks.',
  },
  {
    key: 'editor',
    label: 'Editor',
    bg: 'bg-gray-secondary',
    desc: 'Can create and edit tasks, meetings, and docs.',
  },
  {
    key: 'readonly',
    label: 'Read-only',
    bg: 'bg-gray-light',
    desc: 'View-only access across the project.',
  },
]

export default function ProjectPermissionDropdown({
  anchorRect,
  current,
  onClose,
  onSave,
}: Props) {
  const [selected, setSelected] = useState<ProjectRoleDb>(current)
